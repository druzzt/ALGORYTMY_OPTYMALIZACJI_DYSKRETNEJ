...
#include "headers2.h"
/* PROTOTYPES */
List *newQElem(INTEGER minCost, Vert *vert,
					 List *left, List *right);
List *delQElem(List *queue, Vert *vert);
List *addQElem(List *queue, int minCost, Vert *vert);
Vert *extractMin(List *queue);
Vert* initVert(INTEGER id);
Adjacent *initAdjacent(INTEGER parentId, 
							INTEGER cost, 
							Vert *vert);
Adjacent *addNewAdjacent(Adjacent *adjacent, 
							  Adjacent *newConnection);
Vert **initAllVertices(Vert **verts);
/*
...
Vert **dijkstra(Vert **verts, INTEGER s)
{
	List *queue = NULL;
	verts[s-1]->d = 0;
	queue = addQElem(queue, verts[s-1]->d, verts[s-1]);
	while(queue != NULL) {
		Vert *minCostVert = extractMin(queue);
...
Vert **dijkstraP2P(Vert **verts, INTEGER s, INTEGER t) {
	List *queue = NULL;
	verts[s-1]->d = 0;
	queue = addQElem(queue, verts[s-1]->d, verts[s-1]);
	while(queue != NULL) {
...
Vert **readFiles(char *dataFile) {
   	...
   	while (fscanf(file, "%c", &lineType) != EOF)
	{
		if(lineType == 'c') fgets(buffer, sizeof(buffer), file);
			...
			fscanf(file, " sp "I_FORMAT" "I_FORMAT"", &qVerts, &qArcs);
			graph = (INTEGER**)malloc(qArcs*3*sizeof(INTEGER));
...
INTEGER **readPairs(char *dataFile) {
   	...
   	while (fscanf(file, "%c", &lineType) != EOF)
	{
		if(lineType == 'c') fgets(buffer, sizeof(buffer), file);
			...
			fscanf(file, " aux sp p2p "I_FORMAT"", &qPairs);
			pairs = (INTEGER**)malloc(qPairs*2*sizeof(INTEGER));
		}
		else if(lineType == 'q') {
...
INTEGER *readSources(char *dataFile) {
   	...
   	while (fscanf(file, "%c", &lineType) != EOF)
	{
		if(lineType == 'c') fgets(buffer, sizeof(buffer), file);
			...
			fscanf(file, " aux sp ss "I_FORMAT"", &qSources);
			sources = (INTEGER*)malloc(qSources*sizeof(INTEGER));
		}
		else if(lineType == 's') {
