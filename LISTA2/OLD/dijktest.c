...
void dijkstra(struct Graph* graph, int src)
{
    ...
    int *dist= malloc((V+1) * sizeof(int));      // dist values used to pick minimum weight edge in cut
    ...
    for (int v = 0; v < V; ++v)
    {
        ...
        minHeap->pos[v] = v;
        printf("%d - %d - %d - %d\n",v,dist[v],minHeap->array[v],minHeap->pos[v]);
    }
    ...
    while (!isEmpty(minHeap))
    {
        ...
        while (pCrawl != NULL)
        {
            ...
            // here is segfaults
            printf("0]%llu \t1] %llu \t2] %llu \t3] %llu \t4] %llu \t5] %llu\n",v,dist[u],pCrawl->weight, pCrawl->weight+dist[u], dist[v]);
