...
int *bucketRange(int Cost){
	...
	maxRange++;
	int *r = calloc(maxRange,sizeof(int));
	int j;
...
/** @complexity: O(V)
*/
Bucket *initBucketZ(Bucket *q, INTEGER *di, INTEGER *pi){
	int i;
	q=calloc(V,sizeof(Bucket));
	for(i=0;i<V;i++){
		q[i].idx =i;
		q[i].nr = -1;
		di[i]=INF;
...
int selectNode(Bucket **b, int it){
	for(it=it;it<logC && b[it]==NULL; it++)
		if(it==logC) return it;
	return it;
}

Vertex **radixheap(Vertex **verts, INTEGER s){
	printf(">"I_FORMAT"\n",s);
	Bucket **bucket = calloc(logC,sizeof(Bucket*));
	INTEGER *di;
	INTEGER *pi;
	di = calloc(V,sizeof(INTEGER));
	pi = calloc(V,sizeof(INTEGER));
	Bucket *queue; queue = initBucketZ(queue,di,pi);
	...
	for(labeled=0; labeled < V; labeled++){
		...
		ifContainsOnlyOneVertex(iterator,ranges,bucket,next){
			...
			extractMinVertexFromBucket(tmp,bucket,iterator,idx,min,di,no,int);
		...
		int min_vertex_nr = minimal->idx;
		delBel(&bucket[iterator],&queue[min_vertex_nr], iterator);
		Node* n;
		modificationOfBuckets8(INTEGER,n,gra,min_vertex_nr,
			next,cost,nr,di,var1,pi,dest,ranges,bucket,queue,printf,exit,delBel,addBel);
...
Bucket** initBucketB(Bucket** b){
	return b=calloc(logC,sizeof(Bucket*));
}
Bucket* initBucketQ(Bucket* q){
	return q=calloc(V,sizeof(Bucket));	
}
int* makeArray(int* x){
	return x=calloc(V,sizeof(int));
}
void calculateRanges(int r[]){
	int i;
	r[0]=0;
	for(i=0;i<logC-1;i++)
		r[i+1]=1<<i;
...
int radixheapP2P(int begin_node, int end_node){
	//printf("S:%d \tT: %d\n",begin_node,end_node);
    /* ----------- Initialization ----------- */
    Bucket** b;  Bucket* q;
    b=initBucketB(b);
    ...
    d=makeArray(d);
    p=makeArray(p);
    initBuckets8(int, d,p,q,V,INF);
    int ranges[33];
    calculateRanges(ranges);
    
    /* ---------- Searching for SP ---------- */
    addBel(&b[0], &q[begin_node], 0);
    //printf("\t%d %d %d\n",b[0],q[begin_node],0);
    d[begin_node] = 0;
    int i = 0;
    int k;
    //for(k=0;k<=V;k++) printf("|%d=%d, %d;",k,d[k],p[k]);
    int labeled=0;
    for ( labeled = 0; labeled < V; labeled++) {
       ...
       // printf("{labeled:%d; 1i:%d,",labeled,i);
        ...
        if (! (i == 0 || ranges[i+1] - ranges[i] == 1 || b[i]->next == NULL)) {
            ...
            int no = b[i]->idx;
            int min = d[no];
            for (Bucket* p = b[i]->next; p; p=p->next) {
                if (d[p->idx] < min) {
                    no = p->idx;
                    min = d[no];
                }
...
Vertex **radixheapP2P(Vertex **verts, INTEGER s, INTEGER t){
	printf("S:"I_FORMAT" \tT:"I_FORMAT"\n",s,t);
	int k;
	Bucket **bucket = calloc(logC,sizeof(Bucket*));
	Bucket *queue; queue = initBucketZ(queue);
	//printf("radix");
	for(k=0;k<V;k++){
		//printf(""I_FORMAT" "I_FORMAT" "I_FORMAT" "I_FORMAT"\n",queue[k].idx,queue[k].nr,di[k],pi[k]);
	}
	ranges[32]=INF;
	for(k=0;k<logC-1;k++){
		//printf("%d %d %d %d\n",k,ranges[k+1],ranges[k],1<<k);
	}
	...
	//for(k=0;k<=V;k++) printf("|%d="I_FORMAT", "I_FORMAT";",k,di[k],pi[k]);
	printf("\n");
	for(labeled=0; labeled < V; labeled++){
		//printf(""I_FORMAT"\n",labeled);
		//printf("{labeled:"I_FORMAT"; 1i:"I_FORMAT",",labeled,iterator);
		//iterator = selectNode(bucket,iterator);
		for(k=0;k<32;k++){
			//printf("\t%d "I_FORMAT",\n",k,bucket[k]->idx);
		}
		for(;iterator<logC && bucket[iterator]==NULL; iterator++)
		if(iterator==logC) break;
		//printf("%d; 2i:"I_FORMAT"},",logC,iterator);
		ifContainsOnlyOneVertex(iterator,ranges,bucket,next){
			Bucket *tmp = bucket[iterator]->next;
			extractMinVertexFromBucket(tmp,bucket,iterator,idx,min,di,no,int);
			relabel(iterator,min);
		...
		//update neighbours
		Node* n;
		modificationOfBuckets8(INTEGER,n,gra,min_no,next,cost,
			nr,di,n_no,pi,dest,ranges,bucket,queue,printf,exit,delBel,addBel);
		if(min_no==t)
			break;
	...
	if((argc-1)%2 == 0 &&  argc > 1) {
		...
		if(isP2PMethod) {
			...
			for(i = 0; i < numOfSourceP2P; i++) {
				...
				//printf("%d %d\n",sources[i],outlets[i]);
				printf("PATHPROB=%d\n",PATH_PROB);
				cc = radixheapP2P(sources[i],outlets[i]);}
				//verts = radixheapP2P(verts, pairs[i][0]-1, pairs[i][1]-1);}
				if(PATH_PROB==3){
					fprintf(file, "d "I_FORMAT" "I_FORMAT" "I_FORMAT"\n", 
					sources[i]+1, outlets[i]+1, cc);
				}else{
					fprintf(file, "d "I_FORMAT" "I_FORMAT" "I_FORMAT"\n", 
...
Vertex **readFiles(char *dataFile) {
   	...
   	while (fscanf(file, "%c", &lineType) != EOF)
	{
		if(lineType == 'c') fgets(buffer, sizeof(buffer), file);
			...
			sources=calloc(V,sizeof(INTEGER));
			INTEGER j;
			for(j = 0; j<V; j++){
			...
			nodes[i].idx = b-1;
			nodes[i].cost = cost;
			//printf(""I_FORMAT" "I_FORMAT" \n",nodes[i].idx, nodes[i].cost); // TUTAJ DEBUG
...
INTEGER **readPairs(char *dataFile) {
   	...
   	while (fscanf(file, "%c", &lineType) != EOF)
	{
		if(lineType == 'c') fgets(buffer, sizeof(buffer), file);
			...
			sources=calloc(numOfSourceP2P,sizeof(INTEGER));
			outlets = calloc(numOfSourceP2P,sizeof(int));
		}
		else if(lineType == 'q') {
			pairs[i] = (INTEGER*)malloc(2*sizeof(INTEGER));
			fscanf(file, " "I_FORMAT" "I_FORMAT"", &pairs[i][0], &pairs[i][1]);
			sources[i]=pairs[i][0]-1;
			outlets[i]=pairs[i][1]-1;
			i++;
		}		
		else if(lineType != ' ' && lineType != '\t' && lineType != '\n') {
			printf("Incorrect data!\n");
...
INTEGER *readSources(char *dataFile) {
   	...
   	while (fscanf(file, "%c", &lineType) != EOF)
	{
		if(lineType == 'c') fgets(buffer, sizeof(buffer), file);
			...
			sources = calloc(numOfSeeked,sizeof(INTEGER));
		}
		else if(lineType == 's') {
			INTEGER s;
			fscanf(file, " "I_FORMAT"", &s);
			sources[i]=s-1;
			i++;
		}
		else if(lineType != ' ' && lineType != '\t' && lineType != '\n') {
