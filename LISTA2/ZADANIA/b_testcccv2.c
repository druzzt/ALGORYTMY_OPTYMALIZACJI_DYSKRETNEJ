...
el *tab2list(el *head, struct tablica *tablica, INTEGER E)
{
	...
	int count;
	for(i=1; i<=E; i++){
		//printf("%llu %llu %llu %llu\n", tablica[i], tablica[i].s,tablica[i].t,tablica[i].v);
		ARLOC(lis, el);
 		lis->tab = tablica[i];
 		//printf("^ %llu ^", INT(tablica[i]));
...
int test2(el *head, el3 *Q, struct tablica *tablica, INTEGER S, INTEGER V, INTEGER E)
{
	...
	printf("....");
 	...
 	while(sliQ){
 		sli3=Q;
 		while(sli3){PASSIGNV(tmin,sli3->tab.dist);
 			if(min>=tmin){
 				min=tmin;
...
int main(int argc, char *argv[])
{
    ...
    printf(">%llu<",(INTEGER)itab.V);
 	INTEGER szukane = 1;
 	
	test2(head,Q,itab.tab, szukane, INT(itab.V), INT(itab.E) );
	return 0;
}
