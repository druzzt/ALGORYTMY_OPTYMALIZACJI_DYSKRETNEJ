...
el *tab2list(el *head, Tablica *tablica)
{
	...
	ARLOC(lis, el);
	for(i=0; i<=E; i++){
		printf(" " I_FORMAT "."  I_FORMAT" " I_FORMAT" " I_FORMAT "\n ",i, INT(tablica[i].s),INT(tablica[i].t) ,INT(tablica[i].v) );
		//REARLOC_A(lis, lis2, el,1);
		lis2=lis;	
...
int test2(el *head, el3 *Q, Tablica *tablica1, INTEGER S)
{
	...
	INTEGER i;
	//for(i=1;i<=E;i++)
	//printf("\t{%llu,%llu}\n %llu %llu %llu\n",V,E,tablica[i].s,tablica[i].t,tablica[i].v);
	//head = tab2list(head,tablica1);
	for(i=1; i<=E; i++){
		printf(" " I_FORMAT "."  I_FORMAT" " I_FORMAT" " I_FORMAT "\n",i, INT(tablica1[i].s),INT(tablica1[i].t) ,INT(tablica1[i].v) );
		ARLOC(lis, el);	
 		lis->tab.s = tablica1[i].s;
 		lis->tab.t = tablica1[i].t;
 		lis->tab.v = tablica1[i].v;
 		PUSH_BACK(head,lis);
 	}
 	// DRUKUJE CALA LISTE
 	PRINT(head,elt,printf);
